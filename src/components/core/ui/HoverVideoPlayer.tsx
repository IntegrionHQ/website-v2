  ...
  return (
    <HoverVideoPlayerContext.Provider value={contextValue}>
      <motion.div
        ref={containerRef}
        className={cn(
          "relative overflow-hidden group cursor-pointer w-full aspect-video",
          className
        )}
        style={{
          ...style,
          paddingTop: cropTop ? `${cropTop}%` : undefined,
          paddingBottom: cropBottom ? `${cropBottom}%` : undefined,
        }}
      ...
      >
        {/* Thumbnail - always render if available, hide when playing */}
        {thumbnailSrc && (
...
const HoverVideoPlayerThumbnail: React.FC<{ src: string }> = ({ src }) => {
  const { cropTop, cropBottom, isPlaying } = useHoverVideoPlayer()
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        height: cropTop || cropBottom ? `calc(100% + ${cropTop + cropBottom}%)` : '100%',
      }}
      initial={{ opacity: 1 }}
  ...
  return (
    <div className="flex items-center space-x-2">
      ...
      <Slider
        ...
        max={100}
        step={1}
        onValueChange={(value:any) => setVolume(value[0] / 100)}
        aria-label="Volume"
      />
    </div>
  )
